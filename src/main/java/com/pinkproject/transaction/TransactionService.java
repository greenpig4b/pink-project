...
import com.pinkproject.transaction.TransactionResponse.DailyTransactionDTO.DailyTransaction;
import com.pinkproject.transaction.TransactionResponse.DailyTransactionDTO.DailyTransactionDetail;
...
public class TransactionService {
    ...
    public _DailyMainDTORecord getDailyMain(Integer sessionUserUd, Integer year, Integer month) {
        ...
        // 날짜별로 기록을 그룹화
        Map<String, List<Transaction>> recordsByDate = transactions.stream()
                .collect(Collectors.groupingBy(transaction -> transaction.getCreatedAt().toLocalDate().toString()));

        // 날짜별 기록을 생성하고 정렬
        List<DailyTransaction> dailyTransactions = recordsByDate.entrySet().stream()
                ...
                .map(entry -> {
                    ...
                    // 일별 거래 세부 정보를 생성
                    List<DailyTransactionDetail> dailyTransactionDetails = dailyTransactionList.stream()
                            .map(transaction -> new DailyTransactionDetail(
